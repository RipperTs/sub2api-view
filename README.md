...
- 仅展示用户有效订阅分组内的可调度账号
- 单独页面（自定义菜单）展示账号用量、额度窗口、重置卡和分组等信息（可以一定程度避免号主作弊）
...
### 自定义菜单
使用方法：部署此服务通过 `sub2api` 的自定义菜单功能，填写 `https://example.com/?page=1&page_size=20` ，此时用户点击左侧刚刚新增的菜单可以出现对应用户已有订阅分组对应的账号信息。  

每次调用观察此菜单下的账号最近使用时间， 可以看到是否被转发到其他不知名账号下。
